...
import expert.kunkel.berge.dao.jpa.TourentagDao;
...
public class TourDialog extends javax.swing.JDialog {
	...
	public TourDialog(java.awt.Frame parent, Tour tour) {
		...
		tfBeginn.setDocument(new DocumentDate());
		tfEnde.setDocument(new DocumentDate());
		...
		if (tour != null) {
			...
			try {
				...
				Logger.getLogger(TourDialog.class.getName()).log(Level.SEVERE,
						null, ex);
		...
		tfBeginn.setEditable(isNewTour);
		tfEnde.setEditable(isNewTour);
		cbRegion.setEnabled(isNewTour);

		lblTourentag.setVisible(!isNewTour);
		cbTourentag.setVisible(!isNewTour);
		bTtagBearbeiten.setVisible(!isNewTour);

		List<Karte> karten = karteDao.selectKarte();
		listKarten.setListData(karten.toArray(new Karte[0]));

		if (!isNewTour) {
			List<Tourentag> ttage;
			ttage = tour.getTourentage();

			for (Tourentag ttag : ttage) {
				cbTourentag.addItem(ttag);
			}

			// Karten selektieren
			List<Karte> kartenAuswahl = tour.getKarten();
			for (Karte karte : kartenAuswahl) {
	...
	private void bOKActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bOKActionPerformed
		...
		if (dBeginn == null || cbRegion.getSelectedItem() == null) {
			JOptionPane.showMessageDialog(this,
					"Beginn und Region müssen gefüllt sein!");
			return;
		}
		...
		if (isNewTour) { // für eine neue Tour
			long tage = (dEnde.getTime() - dBeginn.getTime()) / 1000 / 3600
					/ 24 + 1;
			long beginnMillis = dBeginn.getTime();

			for (int i = 0; i < tage; i++) {
				Tourentag tt = new Tourentag();
				tt.setTour(tour);
				tt.setRegion((Region) cbRegion.getSelectedItem());
				tt.setTag(i + 1);
				tt.setDatum(new java.sql.Date(beginnMillis + 1000 * 3600 * 24
						* i));
	...
	private void tourentagBearbeitenActionPerformed(
			java.awt.event.ActionEvent evt) {// GEN-FIRST:event_tourentagBearbeitenActionPerformed
		Tourentag ttag = (Tourentag) cbTourentag.getSelectedItem();
		TourentagDialog ttagDialog = new TourentagDialog(
				(JFrame) this.getParent(), ttag);
	...
	private void bBildergalerieActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bBildergalerieActionPerformed
		GaleriebildDialog bildDialog = new GaleriebildDialog(
				(JFrame) this.getParent(), tour);
	...
	public static void main(String args[]) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				...
				TourDialog dialog = new TourDialog(new javax.swing.JFrame(),
						tour);
	...
	private javax.swing.JList<Karte> listKarten;
	...
	private void setData(Tour tour) throws SQLException, ClassNotFoundException {
		...
		if (tour != null) {
			...
			tfBeginn.setText(sdf.format(tourentage.get(0).getDatum()));
			tfEnde.setText(sdf.format(tourentage.get(tourentage.size() - 1)
					.getDatum()));
			...
			cbGeplant.setSelected(tour.getGeplant());
