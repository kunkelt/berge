...
public class TestRegionDAO extends TestCase {
	...
	private static final String[] names = { "216", "217", "215", "214" };
	private static DaoFactory factory = DaoFactory.getInstance();

	/*
	 * public void testReadOVL_AndUpdateDB() throws IOException, SQLException,
	 * ClassNotFoundException { Region region = getRegion(ID_REGION);
	 * 
	 * RandomAccessFile raf = new RandomAccessFile(PATH_POLYGONE_NICHT_IN_DB +
	 * FILENAME + ".ovl", "r");
	 * 
	 * // erste Zeile auf Symbol testen String firstLine = raf.readLine(); if
	 * (!firstLine.startsWith("[Symbol")) { throw new
	 * IOException("Fehler in Datei! Symbol-Markierung nicht gefunden."); } //
	 * Typ aus zweiter Zeile auslesen String typ = raf.readLine().substring(4);
	 * 
	 * if (Integer.parseInt(typ) != Symbol.SYMBOL_TYP_FLAECHE) { throw new
	 * IOException("Fehler in Datei! Typ ist nicht Flaeche"); }
	 * 
	 * OVL_Flaeche flaeche = new OVL_Flaeche(raf); Polygon polygon =
	 * flaeche.getAsPolygon();
	 * 
	 ...
	 * 
	 * RandomAccessFile raf = new RandomAccessFile(PATH_KANTEN + "Kanten.ovl",
	 * "rw");
	 * 
	 * for (int j = 0; j < entities.size(); j++) { EntityType entity =
	 * entities.get(j); LineString ls =
	 * TtqvUtilities.convertPointsToLineString(entity.getPoints()); OVL_Linie
	 * linie = new OVL_Linie(1,1,1,104,1,ls.getCoordinates());
	 * 
	 * raf.writeBytes("[Symbol "+(j+1)+"]\r\n"); linie.write(raf); }
	 * 
	 * raf.writeBytes("[Overlay]" + "\r\n"); raf.writeBytes("Symbols=" +
	 * entities.size() + "\r\n"); raf.writeBytes("[MapLage]" + "\r\n");
	 * raf.writeBytes("MapName=" + FILENAME + "\r\n");
	 * raf.writeBytes("DimmFc=100" + "\r\n"); raf.writeBytes("ZoomFc=100" +
	...
	/*
	 * public void testReadDBAndUpdateDB() throws Exception { DAOFactory factory
	 * = DAOFactory.getDAOFactory(DAOFactory.POSTGRESQL); RegionDAO regionDao =
	 * factory.getRegionDAO(); List<Region> regionen = regionDao.selectRegion();
	 * 
	 * for (int i = 0; i < regionen.size(); i++) { Region region =
	 * regionen.get(i); updateRegion(region); } }
	 * 
	 * public void testReadTtqvAndWriteShape() throws Exception { Ttqv ttqv =
	 * TtqvUtilities.readTTQV_XML(PATH_KANTEN + "Kanten.xml"); LineString ls =
	 * TtqvUtilities.convertTtqvXML_ToLineString(ttqv, "001");
	 * ShapeUtilities.createShapeFile(ls); }
	 * 
	 * public void testReadDB_AndWriteShapePolygon() throws Exception { Polygon
	 * polygon = getRegion(ID_REGION).getExtent();
	 * ShapeUtilities.createShapeFile(polygon); }
	 */
	private Region getRegion(int id) {
		RegionDao regionDao = factory.getRegionDAO();
		return regionDao.findById(new Integer(id));
	}

	private Region updateRegion(Region region) throws SQLException,
